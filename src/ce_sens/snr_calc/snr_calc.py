...
def snr_calc():
    ...
    parser.add_argument("dynamic", help="if psd is dynamic, true")
    ...
    param_list = get_parameter_list(input_path, apx, df_max, low_freq)
    lenn = len(param_list)
    ...
    if dynamic:
        ...
        for i in tqdm(range(lenn)):
            mer_t = merger_time(param_list[i])
            if mer_t > 0:
                snr = opt_df_dynamic(param_list[i], det, psd_dic, dynamic_psd_dic, lag, switch_duration)
            else:
                snr = opt_df_static(param_list[i], det, psd_dic)
            snr_list.append(snr)

    else:
        print('It is not dynamic')
        snr_list = []
        psd = read_psds(psd_path, df_max, low_freq)
        for i in tqdm(range(lenn)):
            snr = opt_df_static(param_list[i], det, psd)
            snr_list.append(snr)
